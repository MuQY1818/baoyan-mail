...
- 2026-07-17：修复保研信息平台在同一段落并列多组通知时只解析第一组的问题，解析器改为逐个匹配 deadline 与后续通知链接，补齐清华大学深圳国际研究生院和同济大学国优计划等并列记录，并增加多记录段落回归测试。
- 2026-07-27：自动源扩展为保研信息平台、星刻保研和保研岛；小时抓取迁至 GitHub Actions，Worker 改为 staging 分批接收与原子发布。新增具体官方链接一级合并、标题保守二级合并、通用报名门户保护、来源观测与 DDL 冲突保留、数量骤降保护、每源健康统计和 Luna High 分页官方核验接口。
- 2026-07-28：强化多源去重，URL 匹配忽略 HTTP/HTTPS 与 `www` 差异，补充查询参数、数字文章路径和 SPA 详情页识别；增加受控校名别名、固定标题套话指纹、已有多源佐证的同源别名合并，并立即隐藏已被新合并项覆盖的宽限期旧快照。
