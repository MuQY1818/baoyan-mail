...
- 同一官方通知对应多个院系/联合培养项目时，审核池按通知归组并保存全部 `relatedProjects`；主快照仍逐项目保留。共享审核项禁止直接批准成一个人工项目，避免覆盖其他项目。生产回归覆盖 168 个待核验项目对应 167 个通知的情况。
...
2026-09-05 本地验证：类型检查通过，5 个测试文件、196 项测试全部通过，前端构建、`git diff --check` 和本地认证助手语法检查通过。主入口约 263.61 kB（gzip 83.74 kB），申请和日历已分为独立 chunk。
